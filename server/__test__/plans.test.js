...
describe("Plan API", () => {
  ...
  test("GET /plans/:id - get plan by id", async () => {
    const res = await request(app).get(`/plans/${planId}`);
    expect(res.statusCode).toBe(200);
  ...
  test("PUT /plans/:id/feedback - regenerate aiFeedback", async () => {
    const res = await request(app).put(`/plans/${planId}/feedback`);
    expect(res.statusCode).toBe(200);
    expect(res.body).toHaveProperty("aiFeedback");
  });

  test("GET /plans/:id - fail (plan not found)", async () => {
    const res = await request(app).get("/plans/99999");
    expect(res.statusCode).toBe(404);
    expect(res.body).toHaveProperty("message");
  });

  test("PUT /plans/:id - fail (plan not found)", async () => {
    const res = await request(app).put("/plans/99999").send({ judulBelajar: "Tidak Ada" });
    expect(res.statusCode).toBe(404);
    expect(res.body).toHaveProperty("message");
  });

  test("POST /plans - fail (missing judulBelajar)", async () => {
    const res = await request(app).post("/plans").send({ userId });
    expect(res.statusCode).toBe(400);
    expect(res.body).toHaveProperty("message");
  });

  test("PUT /plans/:id - fail (missing judulBelajar)", async () => {
    const res = await request(app).put(`/plans/${planId}`).send({});
    expect(res.statusCode).toBe(400);
    expect(res.body).toHaveProperty("message");
  });

  test("DELETE /plans/:id - delete plan", async () => {
    const res = await request(app).delete(`/plans/${planId}`);
    expect(res.statusCode).toBe(200);
    expect(res.body).toHaveProperty("message");
  });

  test("DELETE /plans/:id - fail (plan not found)", async () => {
    const res = await request(app).delete("/plans/99999");
    expect(res.statusCode).toBe(404);
    expect(res.body).toHaveProperty("message");
  });
  ...
  test("PUT /plans/:id/feedback - fail (force gemini error)", async () => {
    ...
    const res = await request(app).put(`/plans/${aiErrorPlanId}/feedback`);
    expect(res.statusCode).toBeGreaterThanOrEqual(400);
    expect(res.body).toHaveProperty("message");
  });

  test("PUT /plans/:id/feedback - fail (plan not found)", async () => {
    const res = await request(app).put("/plans/99999/feedback");
    expect(res.statusCode).toBe(404);
    expect(res.body).toHaveProperty("message");
  });
});
